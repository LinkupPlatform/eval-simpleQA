...
async def run_linkup_policy(question: str) -> Tuple[str, None]:
    ...
    with ThreadPoolExecutor() as pool:
        result = await loop.run_in_executor(
            pool,
            lambda: LinkupClient(api_key=linkup_api_key).search(
                question, depth="deep", output_type="sourcedAnswer"
            ).answer
        )
        return result, None
...
async def run_linkup_standard_policy(question: str) -> Tuple[str, None]:
    ...
    with ThreadPoolExecutor() as pool:
        result = await loop.run_in_executor(
            pool,
            lambda: LinkupClient(api_key=linkup_api_key).search(
                question, depth="standard", output_type="sourcedAnswer"
            ).answer
        )
        return result, None
...
def generate_question_id(question: str) -> str:
    ...
    return hashlib.sha256(question.encode()).hexdigest()[:16]  # First 16 chars of hash is sufficient
            ...
            for attempt in range(retries):
                ...
                try:
                    predicted_answer, _ = await asyncio.wait_for(
                        run_policy_async(problem, policy_type),
                        timeout=300
                    )
